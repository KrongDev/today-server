...
public interface ScheduleJpaRepository extends JpaRepository<ScheduleEntity, Long> {
    ...
    @Query("SELECT s FROM ScheduleEntity s JOIN ScheduleParticipantEntity sp ON s.id = sp.schedule.id WHERE sp.user.id = :userId")
    ...
    @Query("SELECT s FROM ScheduleEntity s JOIN ScheduleParticipantEntity sp ON s.id = sp.schedule.id " +
            "WHERE sp.user.id = :userId AND s.startTime >= :startDate AND s.endTime <= :endDate")
